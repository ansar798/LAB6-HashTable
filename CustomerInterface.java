...
public class CustomerInterface {
	...
	while(inputfile2.hasNextLine())
	{
		...
		for (int i = 0; i < totalfunds; i++) {
			...
			mutuFundAcc.add(new MutualFundAccount(mf, numShares));
		}
	...
	Customer tempcstmr = new Customer(email, password);
	if(!customers.contains(tempcstmr))
	{
	...
	Customer cstmr = customers.get(tempcstmr);
	System.out.println("\nWelcome, " + first + " " + last + "!\n");
