...
public boolean addFund(double shares, MutualFund mf) {
	...
	if (Double.compare(cash, mf.getPricePerShare()) < 0) {
		...
		MutualFundAccount addFunds = new MutualFundAccount(mf, shares);
		MutualFundAccount name_BST = funds_name.search(addFunds, nC);
		if (name_BST != null) {
			funds_value.remove(name_BST, vC);
			name_BST.updateShares(shares);
			funds_value.insert(name_BST, vC);
		} else {
			funds_name.insert(addFunds, nC);
			funds_value.insert(addFunds, vC);	
		}
